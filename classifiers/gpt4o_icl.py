...
def collect_images_for_task(task_name):
    ...
    originals = (
        dft[dft["Study_ID"].str.contains("_o_")]
        .sample(frac=1, random_state=42)
        .reset_index(drop=True)
    )
    toptiles = (
        dft[dft["Study_ID"].str.contains("_t_")]
        .sample(frac=1, random_state=42)
        .reset_index(drop=True)
    )
...
def get_few_shot_examples(task_name, scenario_df, current_study_id, n_per_class=2):
    ...
    for cls in classes:
        candidates = scenario_df[
            (scenario_df["Ground_Truth"] == cls)
            & (scenario_df["Study_ID"] != current_study_id)
        ]
...
def create_message_list(task_name, examples, target_image_path, scenario="wsi"):
    ...
    if len(ex_classA) < N_PER_CLASS or len(ex_classB) < N_PER_CLASS:
        logging.warning(
            f"Not enough examples for task {task_name} in scenario {scenario}."
        )

    system_message = {"role": "system", "content": BASE_INSTRUCTIONS}

    task_prompt = (
        PROMPT_DICT_WSI[task_name]
        if scenario == "wsi"
        else PROMPT_DICT_TOPTILES[task_name]
    )
...
for task in tasks_of_interest[:3]:
    ...
    for rec in orig_images:
        ...
        for run_i in range(1, N_RUNS + 1):
            ex = get_few_shot_examples(
                task, originals_df, study_id, n_per_class=N_PER_CLASS
            )
            ...
            try:
                ...
                if json_str_start != -1 and json_str_end != -1:
                    ...
                    logger.warning(
                        f"JSON not found in response for {study_id} Run {run_i}"
                    )
            except json.JSONDecodeError as e:
                logger.error(f"JSON parsing error for {study_id} Run {run_i}: {e}")
            except Exception as e:
                logger.error(
                    f"Unexpected error parsing response for {study_id} Run {run_i}: {e}"
                )
    ...
    for rec in top_images:
        ...
        for run_i in range(1, N_RUNS + 1):
            ex = get_few_shot_examples(
                task, toptiles_df, study_id, n_per_class=N_PER_CLASS
            )
