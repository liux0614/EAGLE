...
def train_classifier_lr(X, y, num_runs=10, n_cases=[1, 2, 4, 8, 16, 32], path=None, task=None):
    ...
    for n in n_cases:
        for run in range(num_runs):
            ...
            sample_patient_ids, sample_labels = stratified_sample(patient_ids, patient_to_label, n, random_state=RANDOM_SEED + run)
            X_train_array = [X[pid] for pid in sample_patient_ids]
            y_train_array = label_encoder.transform([sample_labels[pid] for pid in sample_patient_ids])
            label_mapping = {i: label for i, label in enumerate(label_encoder.classes_)}

            X_test_patient_ids = [pid for pid in patient_ids if pid not in sample_patient_ids]
            X_test_array = [X[pid] for pid in X_test_patient_ids]
            y_test_array = label_encoder.transform([patient_to_label[pid] for pid in X_test_patient_ids])

            clf = LogisticRegression(max_iter=10000, solver='lbfgs', random_state=RANDOM_SEED, class_weight='balanced')
            ...
            train_df = pd.DataFrame({'PATIENT': sample_patient_ids, 'LABEL': [sample_labels[pid] for pid in sample_patient_ids]})
            train_df.to_csv(os.path.join(run_path, 'train.csv'), index=False)

            test_df = evaluate_model(clf, np.array(X_test_array), y_test_array, X_test_patient_ids, label_encoder, label_mapping, task)
            test_df.to_csv(os.path.join(run_path, 'internal_test_results.csv'), index=False)
...
def evaluate_model(clf, X_test_array, y_test_array, patient_ids, label_encoder, label_mapping, task):
    ...
    if len(probs) != len(y_test_str):
        raise ValueError(f"Inconsistent number of predictions ({len(probs)}) and true labels ({len(y_test_str)})")
...
def run_logistic_regression_benchmark(model_path, table_dir, model_dir, cohorts, tasks, external_task_mapping, model_name):
    for cohort in tqdm(cohorts, desc="Cohort", leave=False):
        for task in tasks[cohort]:
            ...
            if missing_ids:
                valid_indices = [i for i, pid in enumerate(patient_ids) if pid not in missing_ids]
                patient_ids = [patient_ids[i] for i in valid_indices]
                y = [y[i] for i in valid_indices]
            
            classifiers, label_mapping, label_encoder = train_classifier_lr(
                X, y, path=os.path.join(model_dir, model_name, cohort, task),
                n_cases=[1, 2, 4, 8, 16, 32], task=task
            )
            ...
            if cohort in external_task_mapping and task in external_task_mapping[cohort]:
                for external_cohort in external_task_mapping[cohort][task]:
                    external_clini_table_path = os.path.join(table_dir, f'{external_cohort}.csv')
                    if not os.path.exists(external_clini_table_path):
                        tqdm.write(f"Skipping {external_cohort}: No clinical table found.")
                        continue
                    external_clini_table = pd.read_csv(external_clini_table_path)
                    if task not in external_clini_table.columns:
                        tqdm.write(f"Skipping {external_cohort}: Task {task} not found in clinical table.")
                        continue
                    ...
                    external_X, missing_ids = load_features(external_h5_file, external_patient_ids)
                    if missing_ids:
                        valid_indices = [i for i, pid in enumerate(external_patient_ids) if pid not in missing_ids]
                        external_patient_ids = [external_patient_ids[i] for i in valid_indices]
                    ...
                    for n in [1, 2, 4, 8, 16, 32]:
                        for run_idx, clf in enumerate(classifiers[n]):
                            ...
                            external_results_path = os.path.join(model_dir, model_name, external_cohort, task, run_name)
                            os.makedirs(external_results_path, exist_ok=True)
                            test_df = evaluate_model(clf, external_X_array, external_y_array, external_patient_ids, label_encoder, label_mapping, task)
                            test_df.to_csv(os.path.join(external_results_path, 'external_test_results.csv'), index=False)
