...
def train_classifier(
    model_class, X_train, y_train, patient_ids, task, cohort, num_folds=5, path=None
):
    ...
    for fold, (train_val_index, test_index) in enumerate(
        tqdm(
            kf.split(indices, y_train_encoded), total=num_folds, desc=f"Training {task}"
        )
    ):
        ...
        pd.DataFrame({"PATIENT": train_patient_ids_real, "LABEL": y_train_real}).to_csv(
            os.path.join(fold_path, "train.csv"), index=False
        )
        pd.DataFrame({"PATIENT": val_patient_ids_real, "LABEL": y_val_real}).to_csv(
            os.path.join(fold_path, "valid.csv"), index=False
        )
        pd.DataFrame({"PATIENT": test_patient_ids, "LABEL": y_test_split}).to_csv(
            os.path.join(fold_path, "test.csv"), index=False
        )

        model = (
            model_class(num_classes=len(label_encoder.classes_), input_dim=input_dim)
            .to(device)
            .float()
        )
        ...
        train_ds = [
            (
                torch.tensor(train_X[i], dtype=torch.float32),
                torch.tensor(
                    label_encoder.transform([y_train_real[i]])[0], dtype=torch.long
                ),
            )
        ...
        valid_ds = [
            (
                torch.tensor(val_X[i], dtype=torch.float32),
                torch.tensor(
                    label_encoder.transform([y_val_real[i]])[0], dtype=torch.long
                ),
            )
        ...
        test_ds = [
            (
                torch.tensor(test_X[i], dtype=torch.float32),
                torch.tensor(
                    label_encoder.transform([y_test_split[i]])[0], dtype=torch.long
                ),
            )
        ...
        class_weights = compute_class_weight(
            "balanced", classes=np.unique(y_train_real), y=y_train_real
        )
        ...
        valid_dl = DataLoader(
            valid_ds, batch_size=1, shuffle=False, num_workers=8, pin_memory=True
        )
        test_dl = DataLoader(
        ...
        learn = Learner(
            ...
            cbs=[
                SaveModelCallback(
                    monitor="valid_loss", fname=os.path.join(fold_path, "best_valid")
                ),
        ...
        internal_test_df = predict_and_save_internal(
            learn.model,
            test_dl,
            test_patient_ids,
            y_test_split,
        ...
        internal_test_df.to_csv(
            os.path.join(fold_path, "patient-preds.csv"), index=False
        )
...
def predict_and_save_internal(
    model, test_dl, patient_ids, y_test, task, label_encoder, path
):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    df = pd.DataFrame(
        columns=["PATIENT", task, "pred"]
        + [f"{task}_{cls}" for cls in label_encoder.classes_]
        + ["loss"]
    )
    with torch.no_grad():
        for i, (x, target) in enumerate(
            tqdm(test_dl, total=len(test_dl), desc=f"Internal Testing {task}")
        ):
            x = x.to(device).float()
            ...
            data_row = {
                ...
                "pred": real_pred,
                **{
                    f"{task}_{cls}": probabilities[0][j].item()
...
def predict_and_save(
    ...
    columns = (
        ["PATIENT", task, "pred"]
        + [f"{task}_{cls}" for cls in np.unique(y_test)]
        + ["loss"]
    )
    label_encoder = LabelEncoder()
    ...
    for fold, classifier in enumerate(classifiers):
        ...
        with torch.no_grad():
            ...
            for i, (x, target) in enumerate(
                tqdm(test_dl, total=len(test_dl), desc=f"Predicting {task}")
            ):
                x = x.to(device).float()
                target = target.to(device)
        ...
        save_path = os.path.join(
            path, "deploy", save_name, f"fold-{fold}", "patient-preds.csv"
        )
    ...
    for model in models:
        ...
        for cohort in cohorts:
            for task in tasks[cohort]:
                ...
                path = os.path.join(
                    model_dir, combined_model_name, cancer_type, task.lower()
                )

                # Check if external prediction files already exist
                skip_task = True
                for external_cohort in external_task_mapping.get(cohort, {}).get(
                    task, []
                ):
                    ext_cohort = (
                        "cptac"
                        if "cptac" in external_cohort.lower()
                        else external_cohort
                    )
                    for fold in range(5):
                        fold_pred_path = os.path.join(
                            path, "deploy", ext_cohort, f"fold-{fold}"
                        )
                ...
                if skip_task:
                    print(
                        f"Skipping {task} in {cohort}: All external prediction files already present."
                    )
                    continue
                ...
                patient_ids = (
                    clini_table.loc[valid_rows, "PATIENT"].astype(str).tolist()
                )
                ...
                classifiers = train_classifier(
                    Classifier, X_train, y, patient_ids, task, cohort, path=path
                )
                ...
                if (
                    ...
                    for external_cohort in external_task_mapping[cohort][task]:
                        external_clini_table_path = os.path.join(
                            external_table_dir, f"{external_cohort}.csv"
                        )
                        ...
                        external_y = (
                            external_clini_table.loc[valid_rows, task]
                            .astype(str)
                            .tolist()
                        )
                        external_h5_file = os.path.join(
                            model_path, f"{external_cohort}.h5"
                        )
                        external_X_dict, missing_ids = load_features(
                        ...
                        if missing_ids:
                            ...
                            external_patient_ids = [
                                external_patient_ids[i] for i in filtered_indices
                            ]
                            external_y = [external_y[i] for i in filtered_indices]
                        external_X = [
                            external_X_dict[pid] for pid in external_patient_ids
                        ]
                    ...
                    print(
                        f"Skipping task {task} in {cohort}: No valid external cohorts for this task."
                    )
