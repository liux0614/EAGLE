...
folder_model_mapping = {
    ...
    'eaglefm': ['conch', 'virchow-class', 'ctranspath', 'prov-gigapath', 'virchow2-class'],
}
...
def get_topk_tiles(features, attention_raw, k, patient_id):
    ...
    if num_tiles < k:
        print(f"WARNING: Patient {patient_id} has only {num_tiles} tiles, using all available tiles for top-{k}.")
        k = num_tiles
...
def get_chief_ctp_path(cohort, folder):
    ...
    return os.path.join(ctp_base, cohort, 'ctranspath', 'STAMP_raw_xiyuewang-ctranspath-7c998680')
...
for folder in tqdm(folders, desc="Processing Folders"):
    ...
    slide_tables = [f for f in os.listdir(slide_table_dir) if f.startswith('slide_table_') and f.endswith('.csv')]
    ...
    for cohort in tqdm(cohorts, desc=f"Extracting {folder}", leave=False):
        ...
        for m_check in models:
            ...
            with h5py.File(out_file_check, 'r') as f_chk:
                for pid in valid_patients:
                    if pid not in f_chk:
                        print(f"{pid} missing in output file; valid patients: {len(valid_patients)}")
                        already_done_for_this_cohort = False
                        break
        ...
        if already_done_for_this_cohort:
            print(f"Patients in {cohort} for folder {folder} already complete => Skipped")
            continue
        ...
        for patient_id, group in patient_groups:
            ...
            for _, row in group.iterrows():
                ...
                all_ctp_feats_list.append(torch.tensor(ctp_feats, dtype=torch.float32, device=device))
            ...
            patient_chief_attention[patient_id] = (attention_raw, all_ctp_feats_cat, extractor)
            patient_chief_embedding[patient_id] = slide_embeddings_chief

        print(f"Patients in {cohort}: {len(patient_chief_attention)} extracted attention from {ctp_path_chief}")
            ...
            if folder in ['halfmpp', 'kathermpp', '2mpp'] and m in ['chief', 'eagle']:
                        ...
                        for patient_id, group in patient_groups:
                            ...
                            for _, row in group.iterrows():
                                ...
                                with h5py.File(h5_fm, 'r') as ff:
                                    feats = ff['feats'][:]
                                all_fm_feats_list.append(torch.tensor(feats, dtype=torch.float32, device=device))
                            if len(all_fm_feats_list) == 0:
                                continue
                            fm_feats = torch.cat(all_fm_feats_list, dim=0)
                            selected_feats, _ = get_topk_tiles(fm_feats, attention_raw, k, patient_id)
                            slide_embedding_eagle = torch.mean(selected_feats, dim=0)
                            if patient_id not in f_out:
                                f_out.create_dataset(patient_id, data=slide_embedding_eagle.cpu().numpy())
                ...
                with h5py.File(out_file, 'a') as f_out:
                    ...
                    for patient_id, group in patient_groups:
                        ...
                        if len(all_fm_feats_list) == 0:
                            continue
                        fm_feats = torch.cat(all_fm_feats_list, dim=0)
                        selected_feats, _ = get_topk_tiles(fm_feats, attention_raw, k_val, patient_id)
                        slide_embedding = torch.mean(selected_feats, dim=0)
                        if patient_id not in f_out:
                            f_out.create_dataset(patient_id, data=slide_embedding.cpu().numpy())
                ...
                with h5py.File(out_file, 'a') as f_out:
                    ...
                    for patient_id, group in patient_groups:
                        ...
                        selected_feats, top_values = get_topk_tiles(fm_feats, attention_raw, actual_k, patient_id)
