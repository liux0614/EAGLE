...
def load_virchow2_model(device):
    ...
    config = resolve_data_config(
        virchow2_model.pretrained_cfg, model=virchow2_model
    )
...
def generate_visualization(
    patient_id, tiles_by_slide, cache_dir, vis_output_dir
):
    # Build tile grid
    tile_images = []
    ...
    if not tile_images:
        print(
            f"No tile images available for visualization of patient {patient_id}"
        )
        return
    ...
    sum_w_norm, max_h_norm, _, _ = orientation_stats(
        original_images, rotate=False
    )
    sum_w_rot, max_h_rot, _, _ = orientation_stats(
        original_images, rotate=True
    )
    s_norm = compute_scale(
        sum_w_norm, max_h_norm, grid_width, desired_thumb_height
    )
    s_rot = compute_scale(
        sum_w_rot, max_h_rot, grid_width, desired_thumb_height
    )
    ...
    line_width = 2
    for idx, (s_id, coords, (orig_w, orig_h), im_scaled) in enumerate(
        scaled_slides
    ):
...
def main(args):
    ...
    cohorts = [
        f.replace("slide_table_", "").replace(".csv", "") for f in cohort_files
    ]
    print(f"Found cohorts: {cohorts}")

    for cohort in cohorts:
        slide_table_path = os.path.join(
            slide_table_dir, f"slide_table_{cohort}.csv"
        )
        try:
        ...
        cohort_vis_dir = (
            os.path.join(vis_output_dir, cohort) if args.visualize else None
        )

        slide_table["PATIENT"] = slide_table["PATIENT"].astype(str)
        patient_groups = slide_table.groupby("PATIENT")
        print(
            f"Processing cohort '{cohort}' with {len(patient_groups)} patients..."
        )
        for patient_id, group in tqdm(
            patient_groups, desc=f"Cohort {cohort}", leave=False
        ):
            process_patient(
                patient_id,
                group,
