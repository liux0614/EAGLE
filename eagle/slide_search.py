...
for cohort in tqdm(all_cohorts, desc="Cohorts"):
    ...
    if not os.path.exists(embeddings_file):
        print(
            f"Embeddings file for cohort {cohort} not found at {embeddings_file}. Skipping."
        )
        continue
        ...
        for pid in cohort_patient_ids:
            if pid in hf:
                ...
                if embedding.shape[0] != 1280:
                    print(
                        f"Embedding for patient {pid} in cohort {cohort} has incorrect shape {embedding.shape}. Skipping."
                    )
                    continue
                ...
                print(
                    f"Patient ID {pid} not found in embeddings file {embeddings_file}. Skipping."
                )
...
for cohort in all_cohorts:
    ...
    if len(pids) < num_random_patients:
        print(
            f"Cohort {cohort} has less than {num_random_patients} patients. Skipping."
        )
        continue
    ...
    columns = (
        ["Selected_Patient"]
        + [f"Top_{i+1}_Patient" for i in range(top_k)]
